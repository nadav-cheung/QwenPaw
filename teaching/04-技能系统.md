...
> **增强说明**：本文档于 2026-04-30 进行了内容增强，新增 Java 开发者对比小节、QwenPaw 最新资讯、练习题等内容。
...
| QwenPaw Skill | Java 相关概念 | 说明 |
|---------------|---------------|------|
| Skill | `@Bean` / `Plugin` | 技能类似于 Spring 的组件或 Maven 插件 |
| SkillPool | `BeanFactory` | 全局单例技能仓库 |
| Workspace Skill | `@Scope("prototype")` | 每个工作空间独立的技能实例 |
| SKILL.md | `@Configuration` + `application.properties` | 技能元数据和配置 |
| 技能触发词 | `@Qualifier` + 命令模式 | 通过触发词调用技能 |
| 安全扫描 | JAAS / Security Manager | 技能的安全验证机制 |

### 关键差异

| 方面 | Java (Spring) | QwenPaw |
|------|---------------|---------|
| 依赖注入 | 运行时反射注入 | 启动时解析 + 运行时动态加载 |
| 配置方式 | `@Value`, `application.yml` | SKILL.md + `qwenpaw skills config` |
| 生命周期 | `InitializingBean`, `@PostConstruct` | `register()` 方法 |
| 作用域 | singleton / prototype | global / workspace |
| 发布形式 | JAR 包 | SKILL.md + 脚本目录 |

### 代码对比

**Java (Spring) 定义一个服务组件：**
```java
@Component
@Qualifier("pdfProcessor")
public class PdfProcessorService implements InitializingBean {
    @Override
    public void afterPropertiesSet() throws Exception {
        // 初始化逻辑
    }

    public byte[] processPdf(InputStream input) {
        // PDF处理逻辑
    }
}
```

**QwenPaw 定义一个技能 (SKILL.md)：**
```yaml
name: pdf-processor
version: 1.0.0
description: PDF 文档处理技能
...
**Spring Bean 生命周期：**
```
ComponentScan → BeanDefinition → BeanFactory → Instantiation → Dependency Injection → Initialization → Ready
```

**QwenPaw 技能加载流程：**
```
SkillManifest → resolve_skill() → _get_builtin_skill() / _pool_svc.get_skill() / _load_workspace_skill() → register() → Ready
```
...
```bash
qwenpaw skills list
qwenpaw skills install browser_visible
qwenpaw skills status
```
