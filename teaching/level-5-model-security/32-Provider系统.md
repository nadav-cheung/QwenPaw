...
| **前置知识** | [18-智能体架构](../level-3-agent-core/18-智能体架构.md) |
...
| 区域 | 原因 |
|------|------|
| `ProviderManager` 单例 | 全局状态，修改可能影响所有 Provider |
...
# 列出所有 Provider
print(pm.list_providers())

# 获取特定 Provider
provider = pm.get_provider("openai")
print(provider.model_list)
```

**Provider 连接调试**：
```python
from qwenpaw.providers import OllamaProvider
...
| 场景 | 级别 | 格式 |
|------|------|------|
| Provider 加载 | INFO | `loaded provider: {name}` |
| 模型激活 | DEBUG | `activating: {provider}/{model}` |
...
class MyProvider(OpenAIProvider):
    name = "my-provider"
    default_models = ["my-model"]

    def _normalize_base_url(self, url: str) -> str:
        # 处理自定义 URL 格式
        return url.rstrip("/") + "/v1"

    def _openai_compatible_base_url(self, url: str) -> str:
        # 转换为 OpenAI 兼容格式
        return url
```
...
# 测试 Provider 连接
qwenpaw models --test openai

# 添加自定义 Provider
# 在 config.json 中添加 providers.my_provider 配置
```
