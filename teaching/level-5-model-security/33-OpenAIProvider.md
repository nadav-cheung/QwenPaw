...
| **前置知识** | [18-智能体架构](../level-3-agent-core/18-智能体架构.md)、[17-项目结构](../level-2-getting-started/17-项目结构.md) |
...
| 文件 | 难度 | 原因 |
|------|------|------|
| `providers/openai_provider.py` | ⭐ | OpenAI 接口稳定，改动多为新增模型 |
| `providers/ollama_provider.py` | ⭐ | 本地模型接口，简单易改 |
| `providers/retry_chat_model.py` 的重试策略 | ⭐ | 重试逻辑相对独立 |
| `providers/rate_limiter.py` 的限流算法 | ⭐⭐ | 限流逻辑清晰，测试完善 |

### 10.2 危险区域（绝对不要轻易修改）

| 文件/模块 | 危险原因 |
|-----------|----------|
| `provider_manager.py` 的单例初始化 | 错误会导致所有 Provider 实例混乱 |
| `retry_chat_model.py` 的槽位管理 | 错误会导致流式响应死锁或泄漏 |
| `create_model_and_formatter()` 的包装顺序 | 错误会导致 Token 记录失真 |
...
**测试模型调用**
```python
from qwenpaw.providers import create_model_and_formatter

model, formatter = create_model_and_formatter("gpt-4o")
print(f"Model: {model}, Formatter: {formatter}")
```

**测试 Rate Limiter**
```python
from qwenpaw.providers.rate_limiter import RateLimiter

limiter = RateLimiter(max_qpm=60, max_concurrent=5)
...
**Provider 架构原则**：
- 不要修改 Provider 基类的抽象接口，这会导致所有 Provider 实现崩溃
- 新增 Provider 时确保实现所有抽象方法
- Provider 实例缓存由 Manager 统一管理，不要自行缓存

**模型包装原则**：
- `TokenRecordingModelWrapper` 必须在最内层（直接包装原始模型）
- `RetryChatModel` 必须在最外层（处理所有重试和错误）
- 改变包装顺序会导致 Token 统计不准确

**测试要求**：
- Provider 相关修改必须测试所有支持的模型类型
- 流式响应必须测试中断场景（取消、超时）
