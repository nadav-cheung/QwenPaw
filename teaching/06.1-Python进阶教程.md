...
```python
# src/qwenpaw/app/routers/messages.py:12
...
```python
# src/qwenpaw/app/routers/agents.py:11
...
源码路径: `src/qwenpaw/config/config.py:10`

```python
# src/qwenpaw/config/config.py:10
...
```python
# Pydantic 模型常用操作
req = MessageRequest(content="Hello", stream=True)
...
# 从字典创建模型（会自动验证）
req2 = MessageRequest.model_validate({"content": "Hi", "stream": False})

# 从 JSON 字符串创建
req3 = MessageRequest.model_validate_json('{"content":"Hey"}')
```
...
源码路径: `src/qwenpaw/app/runner/task_tracker.py:17-25`

```python
# src/qwenpaw/app/runner/task_tracker.py:17
...
```python
# src/qwenpaw/app/channels/unified_queue_manager.py:25
from dataclasses import dataclass, field
...
```python
# src/qwenpaw/security/tool_guard/approval.py:5
...
class BootstrapHook:
    ...
    async def __call__(self, agent: Any, **kwargs) -> None:
        """当智能体启动时自动调用"""
        await self._initialize_context(agent)
        await self._load_skills(agent)
