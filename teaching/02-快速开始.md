...
| `qwenpaw init` | 初始化配置 |
| `qwenpaw config show` | 显示当前配置（示例命令） |
| `qwenpaw daemon restart` | 重启应用 |
...
- 或使用命令：`qwenpaw models set --provider <provider> --model <model>`
...
|------|-------------------|------|
| `mvn install` / `gradle build` | `pip install qwenpaw` | Python 用 pip 从 PyPI 安装包，无需先编译再安装 |
| `java -jar app.jar` | `qwenpaw app` | QwenPaw 提供 CLI 入口命令，无需 JVM 启动参数 |
| `application.yml` / `application.properties` | `config.json` | QwenPaw 使用 JSON 配置文件，支持交互式初始化 `qwenpaw init` |
| JVM 进程 | Python 进程 | Python 无需预热 JIT，启动快但运行时性能依赖解释器 |
| IDE: IntelliJ + Maven 插件 | IDE: VS Code + Python 插件 | 推荐使用 VS Code 配合 Python 扩展，或 PyCharm 进行开发调试 |

### 关键差异

Python 的依赖管理不需要中央仓库的 `settings.xml`，`pip install` 直接从 PyPI 拉取。QwenPaw 的配置通过 `qwenpaw init` 交互式生成，不需要手动编写 YAML 模板。
...
| 方向 | 章节 | 说明 |
|------|------|------|
| 项目概览 | [01-项目介绍](./01-项目介绍.md) | 了解 QwenPaw 的核心概念 |
