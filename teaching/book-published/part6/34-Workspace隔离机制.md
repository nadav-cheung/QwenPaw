...
| **前置知识** | [项目架构](./../appendix/A3-项目架构.md)、[智能体核心架构](./07-智能体核心架构.md) |
...
| 文件 | 难度 | 原因 |
|------|------|------|
| `workspace/service_manager.py` 的启动顺序 | ⭐⭐ | 只需调整 priority 数值 |
| `service_factories.py` 工厂函数 | ⭐⭐ | 逻辑清晰，修改配置驱动部分即可 |
| `agent_context.py` ContextVar 中间件 | ⭐⭐ | 中间件逻辑相对独立 |

### 14.2 危险区域

| 文件/模块 | 危险原因 |
|-----------|----------|
| `workspace.py` 的 `_register_services()` | 错误导致服务注册顺序混乱 |
| `service_manager.py` 的 `start_all()` | 错误导致服务启动顺序错误，依赖服务未就绪 |
| `reload_agent()` 热重载逻辑 | 错误导致服务中断或状态丢失 |
| `MultiAgentManager.get_agent()` 双重检查锁 | 错误导致并发创建多个实例 |
...
async def test_workspace():
    ws = Workspace(agent_id="test", workspace_dir="/tmp/test_ws")
    await ws.start()
    print(f"Runner: {ws.runner}")
    print(f"Memory: {ws.memory_manager}")
    await ws.stop()
...
**服务注册规范**：
- 新服务必须在 `_register_services()` 中注册
- 必须提供 `ServiceDescriptor` 所有必需字段
- `priority` 必须满足依赖顺序（被依赖者更小）

**热重载规范**：
- 先创建新实例，再停止旧实例（不可颠倒）
- 可复用服务必须设置 `reusable=True`
- 热重载回调必须捕获异常，避免影响其他服务
