...
| 文件 | 难度 | 原因 |
|------|------|------|
| `src/qwenpaw/app/mcp/schema.py` | ⭐ | MCP 消息模式定义，结构清晰 |
| `src/qwenpaw/app/mcp/renderer.py` | ⭐ | 消息渲染逻辑独立 |
| `src/qwenpaw/app/routers/mcp.py` (配置相关) | ⭐⭐ | MCP 路由配置逻辑清晰 |
| `src/qwenpaw/app/mcp/registry.py` | ⭐⭐ | 注册表逻辑相对独立 |

### 8.2 危险区域（修改前必须咨询 Maintainer）

| 文件/模块 | 危险原因 |
|-----------|----------|
| `src/qwenpaw/app/mcp/manager.py` | MCP 管理器是核心协调器 |
| `src/qwenpaw/app/mcp/client.py` | MCP 客户端连接管理复杂 |
| `src/qwenpaw/app/mcp/watcher.py` | 配置监听涉及文件系统和重载逻辑 |
| `src/qwenpaw/app/routers/mcp.py` (核心路由) | API 路由影响 MCP 服务可用性 |
...
import logging
logging.getLogger("qwenpaw.app.mcp").setLevel(logging.DEBUG)
logging.getLogger("qwenpaw.app.mcp.watcher").setLevel(logging.DEBUG)
```

**方法 2：单元测试**
```bash
# 运行 MCP 相关测试
pytest tests/app/mcp/test_manager.py -v
pytest tests/app/mcp/test_client.py -v
pytest tests/app/mcp/test_watcher.py -v
```
...
# 测试 MCP 服务器连接
qwen mcp test <server_name>
```

### 8.4 如何避免破坏架构

**MCP 客户端规则**：
- 客户端重连逻辑必须遵守 MCP 协议规范
- 传输类型（stdio/sse/streamable-http）必须正确处理
- 错误处理必须传播有意义的异常
...
**工具注册规则**：
- MCP 工具注册必须同步到 Agent 工具列表
- 工具参数模式必须正确转换
- 工具调用结果必须正确格式化

**测试要求**：
- 修改 MCP 管理器后，必须运行 `pytest tests/app/mcp/ -v`
- 修改配置监听后，必须测试热重载功能
- 添加新传输类型必须实现完整测试
