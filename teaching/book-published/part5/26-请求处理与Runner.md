...
| 文件 | 难度 | 原因 |
|------|------|------|
| `app/runner/task_tracker.py` 的断线重连逻辑 | ⭐⭐ | 相对独立的状态管理 |
| `app/runner/models.py` 的数据模型定义 | ⭐ | 纯粹的 Pydantic 模型 |
| `app/runner/session.py` 的会话文件操作 | ⭐ | 文件操作逻辑清晰 |

### 10.2 危险区域（绝对不要轻易修改）

| 文件/模块 | 危险原因 |
|-----------|----------|
| `runner.py` 的 `query_handler` 阶段顺序 | 错误会导致请求流程完全错乱 |
| `task_tracker.py` 的流式输出逻辑 | 错误会导致 SSE 连接泄漏 |
| `session.py` 的原子写入 | 错误会导致会话数据损坏 |

### 10.3 调试方法

**测试 Runner 初始化**
```python
from qwenpaw.app.runner import AgentRunner

runner = AgentRunner()
print(f"Runner initialized with {len(runner.agents)} agents")
```
...
tracker = TaskTracker()
tracker.attach("session-123")
print(f"Attached to session")
```
...
session = SafeJSONSession("user-1", "session-1")
await session.save()
```
...
**Runner 架构原则**：
- `query_handler` 的 12 个阶段必须按顺序执行，不要跳过或重排
- 每个阶段处理完后必须 yield 消息，否则前端无法显示进度
- Agent 实例在请求结束时必须正确清理，避免内存泄漏

**TaskTracker 原则**：
- SSE 连接必须正确关闭，否则会导致文件描述符泄漏
- Buffer 大小必须有限制，避免内存耗尽
- 断线重连时必须验证 session_id 合法性

**测试要求**：
- Runner 必须测试完整的请求生命周期
- TaskTracker 必须测试断线重连场景
- Session 必须测试并发写入和损坏恢复
