...
| **前置知识** | [02-快速开始](./../appendix/A2-快速开始.md)、[03-项目架构](./../appendix/A3-项目架构.md) |
...
| 文件 | 原因 |
|------|------|
| `src/qwenpaw/config/store.py` | 配置加载逻辑清晰 |
| `src/qwenpaw/security/secrets.py` | SecretStore 独立简单 |

### 11.2 危险区域（修改前请联系 Maintainer）

| 区域 | 原因 |
|------|------|
| `secrets.py` 密钥存储 | 修改可能泄露密钥 |
| `store.py` 配置加密 | 修改可能破坏已有配置 |

### 11.3 调试方法

**配置调试**：
```python
from qwenpaw.config.store import load_config
config = load_config()
...
**密钥调试**：
```python
from qwenpaw.security.secrets import SecretStore
store = SecretStore()
key = await store.get_master_key()
