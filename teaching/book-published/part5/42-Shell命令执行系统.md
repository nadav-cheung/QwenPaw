...
| 文件 | 难度 | 原因 |
|------|------|------|
| `agents/tools/shell.py` 的换行符处理 | ⭐ | 独立的字符串处理逻辑 |
| `agents/tools/shell.py` 的智能解码 | ⭐ | 纯粹的编码处理逻辑 |
| `agents/tools/shell.py` 的 Windows 进程终止 | ⭐⭐ | 需要理解进程树 |

### 10.2 危险区域（绝对不要轻易修改）

| 文件/模块 | 危险原因 |
|-----------|----------|
| `agents/tools/shell.py` 的超时终止逻辑 | 错误会导致僵尸进程 |
| `agents/tools/shell.py` 的 `start_new_session` | 错误会影响进程组管理 |
| `agents/tools/shell.py` 的命令清理 | 错误会导致命令注入 |
...
print(f"Killed: {result.return_code == -9}")
```

**测试 Windows 转义修复**
```python
from qwenpaw.agents.tools.shell import _sanitize_win_cmd

fixed = _sanitize_win_cmd('echo \\"hello\\"')
print(f"Fixed command: {fixed}")
```

### 10.4 如何避免破坏架构

**进程管理原则**：
- 超时必须使用 SIGTERM → SIGKILL 两阶段终止
- Unix 必须使用 `start_new_session=True` 创建新进程组
- Windows 必须正确处理进程树的终止

**命令安全原则**：
- 所有命令必须经过清理，不能直接执行用户输入
- Windows 转义修复只处理双引号问题，不能修改其他字符
- 不能在命令中插入未经检查的用户数据

**测试要求**：
- Shell 执行必须测试正常命令、超时、取消场景
- 跨平台必须测试 Unix 和 Windows
- 命令清理必须测试各种注入攻击
