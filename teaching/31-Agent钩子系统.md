...
| 项目 | 内容 |
|------|------|
| **学习目标** | 完成本章后，你能够：1) 分析钩子的类型和执行时机 2) 理解实例钩子与全局钩子的区别 3) 实现自定义 Agent 钩子 |
| **前置知识** | [智能体钩子系统](./23-智能体钩子系统.md)、[智能体核心架构](./07-智能体核心架构.md) |
| **预计时长** | 30 分钟（阅读 20 + 练习 10） |
| **难度等级** | ⭐⭐⭐⭐ |
| **核心关键词** | `钩子` `pre_reasoning` `pre_acting` |

> **一句话概述**：Agent 钩子系统通过装饰器模式在推理和行动等关键节点注入自定义逻辑，支持首次引导、上下文压缩等场景，并具备重入保护和优先级排序能力。
...
1. **`pre_reasoning` 和 `pre_acting` 钩子分别在什么时机执行？它们能修改什么数据？**
2. **实例级钩子和类级钩子的执行顺序是什么？为什么实例级钩子先于类级钩子？**
3. **`MemoryCompactionHook` 的重入保护机制是如何实现的？如果不加保护会出现什么问题？**
...
- [智能体钩子系统](./23-智能体钩子系统.md) — 钩子系统的架构设计与扩展点
- [智能体核心架构](./07-智能体核心架构.md) — Agent 的完整生命周期
- [插件系统](./18-插件系统.md) — 插件如何通过钩子机制集成
