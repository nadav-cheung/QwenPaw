...
| 检查项 | 状态 | 证据 |
|--------|------|------|
| `tools/__init__.py` 导出 19 个工具函数 | ✅ | `__all__` 列表包含 19 个条目 — `__init__.py:34-59` |
| `shell.py` 跨平台分支真实存在 | ✅ | `sys.platform == "win32"` 分支使用 `asyncio.to_thread` + `_execute_subprocess_sync`；Unix 使用 `asyncio.create_subprocess_shell` — `shell.py` |
| `shell.py` 进程组管理 | ✅ | Unix 使用 `start_new_session=True` + `os.killpg()` 超时杀进程组 — `shell.py` |
| `shell.py` 换行折叠函数 | ✅ | `_collapse_newlines_outside_quotes()` — `shell.py:40-80` |
| `file_io.py` 跨平台编码 | ✅ | `_get_encoding_for_file()` 对 CSV/TSV/TXT 使用 `utf-8-sig`（Windows Excel 兼容），其他文件用 `utf-8` — `file_io.py:42-63` |
| `file_io.py` 路径解析 | ✅ | `_resolve_file_path()` 处理绝对/相对路径，使用 `get_current_workspace_dir()` context var — `file_io.py:23-39` |
| `edit_file` 使用 `str.replace` 而非 `re.sub` | ✅ | `new_content = content.replace(old_text, new_text)` — `file_io.py:328` |
| `file_search.py` 存在 | ✅ | 628 行，包含 `grep_search` 和 `glob_search` 实现 |
| `browser_control.py` 存在 | ✅ | 3647 行，Playwright 驱动的 `browser_use` 实现 |
| `agent_management.py` 多智能体工具 | ✅ | `list_agents`, `chat_with_agent`, `submit_to_agent`, `check_agent_task` |
| `view_media.py` 媒体查看 | ✅ | `view_image`, `view_video` 函数 |
| `delegate_external_agent.py` ACP 委托 | ✅ | 通过 ACP 协议委托外部 Agent |
| 工具注册在 `_create_toolkit` | ✅ | `react_agent.py:190-307` — `tool_functions` 字典 + `toolkit.register_tool_function` 循环 |
| `async_execution` 支持 | ✅ | 仅 `execute_shell_command` 支持异步执行，且仅从 `builtin_tools` 配置读取 — `react_agent.py:219-225` |
| 任务管理工具条件注册 | ✅ | `view_task`, `wait_task`, `cancel_task` 仅在 `has_async_tools` 时注册 — `react_agent.py:279-305` |

**审查结论**: 所有工具实现路径、函数签名、跨平台分支均与真实源码一致。修正了 `edit_file` 使用 `str.replace` 而非 `re.sub` 的描述。

## 教学审查 (Pedagogy Review)

| 检查项 | 状态 | 说明 |
|--------|------|------|
| 学习目标可验证 | ✅ | 5 个具体目标：理解架构、注册流程、shell 跨平台、文件安全、浏览器控制 |
| 从简单到复杂 | ✅ | 工具注册流程 → shell 命令（最复杂）→ 文件操作 → 浏览器控制 |
| Mermaid 图准确 | ✅ | 工具调用时序图和跨平台状态机均基于真实代码流 |
| 跨平台差异有代码示例 | ✅ | Windows `asyncio.to_thread` + 临时文件 vs Unix `create_subprocess_shell` + PIPE |
| 工程决策有解释 | ✅ | 为什么用临时文件代替管道、为什么需要进程组 |

## 工程审查 (Engineering Review)

| 检查项 | 状态 | 说明 |
|--------|------|------|
| 技术债已识别 | ✅ | 2 项（`smart_decode` 历史遗留、`_collapse_embedded_newlines` LLM 输出处理） |
| 跨平台问题已覆盖 | ✅ | Windows 管道阻塞问题 → 临时文件方案；进程组 → `os.killpg` |
| 安全关注点已注明 | ✅ | Shell 命令注入警告、路径遍历警告 |
| browser_control.py 覆盖度 | ⚠️ | 3647 行代码仅简要提及。建议在后续版本中扩展浏览器控制工具的深度分析 |

## Contributor 审查 (Contributor Review)

| 检查项 | 状态 | 说明 |
|--------|------|------|
| 安全文件清单 | ✅ | 工具文件独立性强，修改单个工具不影响其他 |
| 危险区域标注 | ✅ | Shell 命令注入、路径遍历 |
| 测试方法指引 | ✅ | pytest 示例代码（`test_read_file_basic`, `test_read_file_encoding`） |
| 新增工具流程 | ✅ | 3 步标准流程（创建文件 → 导出 → 注册） |
| 调试方法 | ✅ | 直接调用工具函数 + 检查 `toolkit.get_tool_names()` |

---

*Chapter 20 审查完成。基于 tools/ 目录 19 个工具函数的真实源码。修正 edit_file 实现描述（`str.replace` 而非 `re.sub`）。*
