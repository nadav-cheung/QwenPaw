...
| **前置知识** | [18-智能体架构](./18-智能体架构.md)、[27-请求处理器](../level-4-application/27-请求处理器.md) |
...
```python
import logging
logging.getLogger("qwenpaw.agents.mission").setLevel(logging.DEBUG)
logging.getLogger("qwenpaw.agents.mission.runner").setLevel(logging.DEBUG)
```

**方法 2：单元测试**
```bash
# 运行 Mission 相关测试
pytest tests/agents/mission/test_state.py -v
pytest tests/agents/mission/test_handler.py -v
pytest tests/agents/mission/test_runner.py -v
```
...
# 测试 Mission 执行
qwen mission run <mission_id> --dry-run
```

### 8.4 如何避免破坏架构

**状态转换规则**：
- 状态转换必须通过 `StateManager` 进行
- 状态转换必须记录历史，支持回滚
- 不允许的状态转换必须抛出异常

**Worker 管理规则**：
- Worker Agent 的工具限制必须在 Phase 配置中声明
- Worker 执行超时必须被正确处理
- Worker 失败必须传播到 Mission 级别

**文件持久化规则**：
- Mission 状态文件必须使用原子写入
- 状态文件路径必须与 Mission ID 一一对应
- 清理时必须验证所有相关文件

**测试要求**：
- 修改状态机后，必须运行 `pytest tests/agents/mission/test_state.py -v`
- 修改处理器后，必须运行完整的工作流测试
- 添加新 Phase 必须实现完整的测试覆盖

---

## 延伸阅读

- [请求处理与Runner](./27-请求处理器.md) — Runner 如何检测和启动 Mission Mode
- [任务追踪系统](./27-请求处理器.md) — Mission 执行中的任务追踪机制
- [多智能体协作](./25-多智能体协作.md) — Master Agent 与 Worker 的协作模式
