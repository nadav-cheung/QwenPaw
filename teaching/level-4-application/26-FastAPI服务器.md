...
| 文件 | 原因 |
|------|------|
| `src/qwenpaw/app/routers/messages.py` | 路由处理函数独立，新增端点容易 |
| `src/qwenpaw/app/routers/config.py` | 配置 CRUD 逻辑简单 |
| `src/qwenpaw/app/routers/skills.py` | 技能路由，逻辑清晰 |

### 11.2 危险区域（修改前请联系 Maintainer）

| 区域 | 原因 |
|------|------|
| `routers/__init__.py` 路由聚合 | 修改影响全局路由结构 |
| `agent_scoped.py` 作用域路由 | 修改可能破坏 Agent 隔离机制 |
| `_app.py` 中间件链 | 认证和 Agent 上下文注入逻辑复杂 |
...
# 测试 Agent 作用域路由
from qwenpaw.app.routers.agent_scoped import router as agent_router
print(agent_router.routes)
```
...
# 临时禁用认证
import os
os.environ["QWENPAW_AUTH_ENABLED"] = "false"
...
| 场景 | 级别 | 格式 |
|------|------|------|
| 路由注册 | DEBUG | `registered: {method} {path}` |
| 认证结果 | DEBUG | `auth: {agent_id} {status}` |
| 请求处理 | INFO | `{method} {path} {status_code}` |

### 11.5 架构规范

1. **路由前缀规则**：`/api/*` 为标准 API，`/voice/*` 为语音专用
2. **Agent 作用域路由必须包含 `{agentId}` 参数** — 确保多租户隔离
3. **自定义路由必须以 `/api/` 开头** — 便于统一认证和中间件处理
4. **公开端点列表**：`routers/__init__.py` 中的 `PUBLIC_PATHS` 必须与实现同步

### 11.6 新增路由模板

```python
# src/qwenpaw/app/routers/new_router.py
...
async def get_item(request: Request, item_id: str):
    """获取单个项目"""
    agent_id = request.state.agent_id  # 如果是 agent scoped
    return {"item_id": item_id, "agent_id": agent_id}
```
...
```bash
# 查看所有路由
grep -r "@router\." src/qwenpaw/app/routers/

# 测试 API
curl http://localhost:8088/api/config

# 测试 Agent 作用域
curl http://localhost:8088/api/agents/default/config
```
...
- [17-项目结构](../level-2-getting-started/17-项目结构.md) -- 理解 QwenPaw 项目整体架构
- [27-请求处理器](./27-请求处理器.md) -- 深入理解请求处理流程
...
| 检查项 | 状态 | 说明 |
|--------|------|------|
| 路由注册顺序与文档一致 | ✅ | `routers/__init__.py:29` -- 9 个路由的注册顺序与文档表格完全匹配 |
| Agent 作用域路由前缀可验证 | ✅ | `_app.py:610` -- `prefix="/api"` 和 `prefix="/agents/{agentId}"` 前缀正确 |
| Voice 路由独立于 /api 前缀 | ✅ | `_app.py:625` -- `voice_router` 以根路径注册，未带 `/api` 前缀 |

**审查结论**: 文档描述的路由分层架构与源代码实现完全一致，无偏差。

## 教学审查 (Pedagogy Review)

| 检查项 | 状态 | 说明 |
|--------|------|------|
| 概念递进合理 | ✅ | 从路由组织到注册到中间件链，逐步深入，符合认知规律 |
| 代码示例与讲解比例 | ✅ | 每个概念均配有实际源码路径和行号，便于对照学习 |
| 前置知识覆盖充分 | ✅ | 引用第 17 章项目结构作为基础，第 27 章作为深入延伸 |

## 工程审查 (Engineering Review)

| 检查项 | 状态 | 说明 |
|--------|------|------|
| 路由设计解耦良好 | ✅ | 全局路由、Agent 作用域路由、专用路由三层分离，职责清晰 |
| 中间件链顺序合理 | ✅ | 认证中间件在路由处理前执行，避免未授权请求进入业务逻辑 |
| 路径参数校验 | ⚠️ | `agentId` 路径参数未见显式校验逻辑，建议在后续版本中补充 |

## Contributor 审查 (Contributor Review)

| 检查项 | 状态 | 说明 |
|--------|------|------|
| 文档可维护性 | ✅ | 路由表格结构与代码注册顺序对齐，新增路由时可快速定位修改点 |
| 测试覆盖率 | ✅ | 快速参考部分提供了 `curl` 测试命令和 `grep` 路由查看方式
